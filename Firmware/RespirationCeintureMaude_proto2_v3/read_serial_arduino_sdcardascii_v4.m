...
%% USER INPUT
sddatasize=10;
floatval = [1 1000 1000 1 1 1 1000 1000 1000 1];

recordAllToCSV = 0;% If equal to 1, transfer all acqusitions to CSV
if (recordAllToCSV == 0)%or we can shoose only a few ones to transfer to CSV
    recordAcquisitions = 0; %if you want only given acquisitions
...
for iterateacquisition_list=1:length(recordAcquisitions)
    ...
    %fprintf(fid,'\n');
    command = strcat('readdata'," ",filelist{iterateacquisition}(5:8));
    %fprintf(command,'%s\n',strcat('readdata'," ",filelist{iterateacquisition}(5:8)));
    write(s,convertStringsToChars(command),"string");
    ...
    while(1)
        ...
        an = convertStringsToChars(an2);
            ...
            try
                ...
                readdatasplit = split(readdata(2:end),",");
